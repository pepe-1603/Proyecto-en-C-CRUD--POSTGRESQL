/**
 * REcordar que para enviar mensaje de registro exitoso se utiliza
 * el fifo Model_autos para autos y accidentes pero no para Clientes
 */
...
int main(int argc, char const *argv[])
{
    ...
    conn = PQsetdbLogin("localhost", "5432", NULL, NULL, "aseguradora", "rol_usr1", "rol_usr1");
    if (PQstatus(conn) == CONNECTION_OK)
    {
        ...
        while (!Hecho)
        {
            ...
            if (num_bytes > 0)
            {
                ...
                case 3:
                    printf("Cliente Elijio Administracion de Accidentes");
                    submenu_Accidentes();
                ...
                default:
                    red();
                    printf("\nComando No valido para Menu Pricipal\n");
                    reset();
                    break;
...
void f_registrar(char *sql)
{
    ...
    char msg2[] = "\n\t\t Todo Salio Bien :] ,Registro Exitosa :]\n";
    ...
    if (PQresultStatus(res) == PGRES_COMMAND_OK)
    {
        ...
        fd_write = open(FIFO_Model_autos, O_WRONLY);
...
void f_actualizar(char *sql)
{
    ...
    printf("consulta UPDATE final : %s", sql);
...
void obtener_registros(char *sql)
{
    ...
    registros array_data[N + 1]; // CAPTURAR nUM_FILAS EN pqNtUPLES
    ...
    if (N > 0)
    {
        ...
        write(fd, array_data, N * sizeof(registros));
        close(fd);
        PQclear(res);
...
void getRegistroAutos(char *sql)
{
    ...
    num_rows = PQntuples(res);
    N = num_rows;
    registro_autos data[N + 1]; // CAPTURAR nUM_FILAS EN pqNtUPLES
    ...
    fd = open(FIFO_CLIENTE, O_WRONLY);
    write(fd, data, N * sizeof(registro_autos));
    close(fd);
    PQclear(res);
...
void getRegistrosAccident(char *sql)
{
    ...
    num_rows = PQntuples(res);
    N = num_rows;
    registros_accident datos[N + 1]; // CAPTURAR nUM_FILAS EN pqNtUPLES
    ...
    if (N > 0)
    {
        ...
        fd = open(FIFO_CLIENTE, O_WRONLY);
        write(fd, datos, N * sizeof(registros_accident));
        close(fd);
        PQclear(res);
    ...
    {
        printf("\n---------------------------------------------------------");
        printf("\n  |id_Accidente |  Placa  | fecha |   Lugar | Hora|\n");
        printf("---------------------------------------------------------\n");
